...
onready var cellroute = load("res://Scenes/Cell.tscn")
...
func divide():
	...
	if Gen != 2:
		...
		cell1.global_position = $CellNewSpawn.global_position
		...
		cell2.global_position = $CellNewSpawn2.global_position
...
func _on_AreaDetection_area_entered(area):
	if area.get_parent().type != null:
		match area.owner.type:
			0:
				if rand_range(0,1) < 0.75:
					Gen = 2
				else:
					Gen = int(rand_range(0,4))
			1:
				if rand_range(0,1) > 0.9:
					Gen = 4
				else:
					Gen = int(rand_range(0,3))
			2:
				if rand_range(0,1) < 0.8:
					Gen = 0
				else:
					Gen = int(rand_range(1,4))
		redy()
